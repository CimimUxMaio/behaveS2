...
describe("#Game", function()
	...
	describe("#defer", function()
		it("adds a task to the deferred tasks queue", function()
			local task = function() end
			game:defer(task)
			assert.are.equal(1, #game.deferredTasks)
		end)

		it("multiple tasks can be added before update", function()
		...
		it("if delay is 0, tasks are called on the next update", function()
		...
		it("tasks are called after the given delay", function()
			local task = spy.new(function() end)
			game:defer(task, 2)
			game:update(1)
			assert.spy(task).was_not_called()
			game:update(1)
			assert.spy(task).was_called()
		end)

		it("tasks are not called if not enough time has passed", function()
			local task = spy.new(function() end)
			game:defer(task, 100)
			game:update(10)
			game:update(21)
		...
		it("tasks are called with the given arguments", function()
		...
		it("executed tasks are removed from the deferred tasks queue", function()
			local task = spy.new(function() end)
			game:defer(task, 1)
			assert.are.equal(1, #game.deferredTasks)
			game:update(1)
			assert.spy(task).was_called()
			assert.are.equal(0, #game.deferredTasks)
		end)

		it("not executed tasks are not removed from the deferred tasks queue", function()
			local task = spy.new(function() end)
			game:defer(task, 10)
			assert.are.equal(1, #game.deferredTasks)
			game:update(9)
